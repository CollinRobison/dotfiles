...
def extract_wrapper_maps(text: str, path: Path) -> list[Mapping]:
    ...
    for hit in re.finditer(r'(?<!function\s)\bmap\(', text):
        ...
        if key_values and desc_values:
            out.append(Mapping("Normal", display_key(key_values[0]), desc_values[-1], category(path), path.name, file_context(path)))
    return out
...
def extract_tools() -> list[tuple[str, str]]:
    """Inventory declared plugin/tool identifiers so unmapped tooling is findable too."""
    tools: set[tuple[str, str]] = set()
    for path in sorted(LUA.rglob("*.lua")):
        text = path.read_text(errors="replace")
        for tool in re.findall(r'["\']([\w.-]+/[\w.-]+)["\']', text):
            tools.add((tool, path.name))
    return sorted(tools, key=lambda item: item[0].lower())


def runtime_audit() -> tuple[int, str]:
    """Count described runtime mappings without treating plugin defaults as docs."""
    with tempfile.TemporaryDirectory(prefix="nvim-atlas-") as temp:
        ...
        output = root / "maps.json"
        lua = 'local o={}; for _,m in ipairs({"n","v","x","o","i","s","c","t"}) do for _,k in ipairs(vim.api.nvim_get_keymap(m)) do if k.desc and not k.lhs:match("<Plug>") then table.insert(o,{m=m,l=k.lhs,d=k.desc}) end end end; vim.fn.writefile({vim.json.encode(o)}, os.getenv("ATLAS_RUNTIME_JSON"))'
        env = dict(**__import__("os").environ, XDG_CONFIG_HOME=str(cfg), ATLAS_RUNTIME_JSON=str(output))
        try:
            ...
            return len(rows), "Runtime audit completed"
        except Exception as exc:
            return 0, f"Runtime audit unavailable ({type(exc).__name__}); source inventory remains authoritative"
...
def page_header(title: str, eyebrow: str, count: int) -> str:
    return f"<header><div><p class='eyebrow'>{html.escape(eyebrow)}</p><h1>{html.escape(title)}</h1></div><div class='meta'>Collin’s setup • Leader = Space<br><b>Last regenerated: {date.today().isoformat()}</b><br>{count} configured mappings</div></header><div class='rule'></div>"
...
def build_html(entries: list[Mapping], tools: list[tuple[str, str]], runtime_count: int, runtime_note: str) -> str:
    ...
    for start in range(0, len(tools), tool_chunk_size):
        ...
        suffix = "" if start == 0 else f" · continued ({start + 1}–{start + len(chunk)} of {len(tools)})"
    ...
    for title in sorted(grouped):
        ...
        for start in range(0, len(rows), chunk_size):
            ...
            suffix = "" if start == 0 else f" · continued ({start + 1}–{start + len(chunk)} of {len(rows)})"
